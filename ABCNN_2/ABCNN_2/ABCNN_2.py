...
class ABCNN_2(Chain):
    ...
    def __init__(self, n_vocab, n_layer, embed_dim, input_channel, output_channel, train=True):
        ...
        if self.n_layer == 1:
            super(ABCNN_2, self).__init__(
                embed=L.EmbedID(n_vocab, embed_dim, initialW=np.random.uniform(-0.01, 0.01)),
                # embed=L.EmbedID(n_vocab, embed_dim, initialW=RS.normal(scale=0.01,size=(n_vocab,embed_dim))),
                # embed=L.EmbedID(n_vocab, embed_dim),
                conv1=L.Convolution2D(1, output_channel, (4, embed_dim), pad=(3,0)),
            ...
            super(ABCNN_2, self).__init__(
                embed=L.EmbedID(n_vocab, embed_dim, initialW=np.random.uniform(-0.01, 0.01)),
                conv1=L.Convolution2D(1, output_channel, (4, embed_dim), pad=(3,0)),    
                conv2=L.Convolution2D(1, output_channel, (4, 50), pad=(3,0)),
    ...
    def pad_vec2zero(self,vocab):
        vec = self.xp.array([0.0]*self.embed_dim, dtype=self.xp.float32)
        self.embed.W.data[vocab["<pad>"]] = vec
